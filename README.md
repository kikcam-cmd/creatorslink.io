...
| 4 | Reminders (the retention hook) | **Built — code/build/migration verified; INERT until Cameron sets 3 secrets + verifies the Resend domain in Vercel. Dry-run lever ready; live send unverified** |
...
- **D-021 — Reminders: Vercel Cron + a Node API route + Resend, sending a daily 7-day digest; service-role access; opt-out (default on), no sent-log.** *(2026-05-31)* The retention hook (Phase 4) is a **daily reminder email**. *Decisions inside this:* (a) **Vercel Cron over Supabase pg_cron** — in-repo, Next-native, debuggable; `vercel.json` `crons` hits `/api/cron/reminders` at `0 15 * * *` (15:00 UTC ≈ 8am Pacific; inherits the `lib/dates` UTC ~day-accuracy limitation, not fixed here). (b) **Resend** for transactional mail (team standard; `lib/email.ts`, plain-HTML template, no react-email in v1), sender from `REMINDER_FROM_EMAIL` so the verified-domain choice is config not code. (c) **One daily digest, 7-day lookahead, sent only when non-empty** — mirrors the dashboard's "Due this week" window so the email and the app agree; the Build Plan calls this "the daily-habit hook." (d) **Service-role access** — the cron has no user session, so it uses a bare `@supabase/supabase-js` service-role client (`lib/supabase/admin.ts`, RLS-bypassing, server-only, **never** `NEXT_PUBLIC_`) to read across creators and `auth.admin.listUsers()` for the email (which isn't queryable via PostgREST). (e) **Opt-out, default on** — `profiles.email_reminders boolean default true` (`0007`), a Settings checkbox; the cron filters on it. (f) **No sent-log/dedup** — a daily snapshot is idempotent by construction; resending tomorrow's snapshot is correct. A `List-Unsubscribe` header + a Settings unsubscribe link cover compliance. (g) **Secured + verifiable** — the route requires `Authorization: Bearer ${CRON_SECRET}` (Vercel injects this for scheduled runs; fails closed with 401 if unset), and supports `?dryRun=1` to return the per-creator digest JSON (recipients + counts) **without sending** — the verification lever, since neither a push nor a headless agent can confirm real mail. *This phase is INERT until three Vercel env vars are set (`SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `CRON_SECRET`, plus `REMINDER_FROM_EMAIL`) and the Resend domain is verified.* *Deferred:* per-user send-time/timezone, weekly vs daily preference, and a richer template. *Deferred scale work (fine now, address when approaching ~1,000+ creators — Resend itself stays the right provider well past that; the digest is a conditional daily email so volume is small, and the send is isolated behind `lib/email.ts` so swapping providers later is a one-file change):* (1) the cron sends sequentially — at ~1k recipients × ~150ms this nears Vercel's function timeout, so batch via Resend's batch endpoint (≤100/call) or chunk; (2) `buildDigests` loads all due rows and groups in JS — move to SQL-side aggregation (a `SECURITY DEFINER` RPC) or paginate at high row counts.
...
- **2026-05-31 (Phase 4 — Reminders, built but inert)** — Built the daily reminder digest. `vercel.json` adds a `crons` entry hitting a new Node route `app/api/cron/reminders/route.ts` (Bearer `CRON_SECRET` gate + `?dryRun=1` JSON lever) at 15:00 UTC. `lib/reminders.ts` builds per-creator digests (overdue + due-in-7-days deliverables & payments, reusing the dashboard window + `lib/types` helpers); `lib/supabase/admin.ts` is the RLS-bypassing service-role client (reads across creators + `auth.admin.listUsers()` for emails); `lib/email.ts` sends via Resend (plain HTML, `List-Unsubscribe`). Migration `0007` adds `profiles.email_reminders` (bool, default true) + a Settings opt-out checkbox (wired through `updateProfile`). Added `resend` dep; `.env.example` documents the 4 new vars (templates only — no real secrets). Recorded **D-021**. `next build` + lint green; migration + column verified live; advisors unchanged (1 WARN, 0 ERROR). **Residual:** Cameron sets the 3 secrets + verifies the Resend domain in Vercel, then a `?dryRun=1` check and a real send confirm it end-to-end — until then the phase is inert.
